...
BACKUP_DATA="$HOME/scripts $HOME/.ssh $HOME/randomz /proj"
...
ssh -q -i $SSH_KEY mfski@sverige.freeshell.org -n "find ~/backup/backup-* -type f -exec rm {} \; 2>/dev/null" 2>/dev/null | tee -a $LOG
echo ""
echo "Deleted old backups at remote host..." | tee -a $LOG


## RSYNC BACKUP TO REMOTE SITE
rsync -avz --rsh="ssh -i $SSH_KEY" --delete --numeric-ids $BACKUP mfski@sverige.freeshell.org:~/backup 2>/dev/null | tee -a $LOG
echo ""
